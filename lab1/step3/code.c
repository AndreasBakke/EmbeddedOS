...
DeclareResource(countResource);
volatile int count =	0; //global count variable
		...
		WaitEvent(TimerEvent1); //wait any event? No. why specify event? Ans: event we want to wait. more than one event with bitwise |
				{
					...
					if (mask1 & TimerEvent1) {
						GetResource(countResource);
						count= (count+1)%10;
						ReleaseResource(countResource);
		...
		WaitEvent(TimerEvent2); //wait any event? No. why specify event? Ans: event we want to wait. more than one event with bitwise |
				{
					...
					if (mask2 & TimerEvent2) {
						GetResource(countResource); //lock
						if (count <5){
							//could possibly relase resource here to reduce blocked time. But you should also introduce a releaseResource in an else-statement if the IF statement is not true.
							led_blink(0b10000000); //blink led 7
						}
					ReleaseResource(countResource);//release
